...
public class UserController {
	...
	@PostMapping()
	public ResponseEntity<UserResponse> save(@Valid @RequestBody User usuario) {
	    ...
	    user =  this.userService.save(usuario);
	...
	@PostMapping("/all")
	public ResponseEntity<List<UserResponse>> saveAll(@Valid @RequestBody List<User> users) {
		
		
		List<User> listUsers=users.stream()
		 ...
		 if(listUsers.isEmpty()) {
	    	...
	    	listUsers= users.stream().map((user)->{
	    	...
	    	return new ResponseEntity<List<UserResponse>>(users.stream().map(user-> this.convertToDto(user)).collect(Collectors.toList()), HttpStatus.CREATED);
	     }
